...
# %%
model_names = {
    "1.5b": "deepseek-ai/DeepSeek-R1-Distill-Llama-8B", #"meta-llama/Llama-3.1-8B-Instruct", #"Qwen/Qwen2.5-7B-Instruct", #"deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B",
    "32b": "Qwen/QwQ-32B", #"simplescaling/s1.1-32B", #"Qwen/QwQ-32B", #"simplescaling/s1.1-32B", #"Qwen/QwQ-32B",
}
ports = {
    "1.5b": int(os.getenv("SGLANG_PORT_1_5B", "40004")),
    "32b": int(os.getenv("SGLANG_PORT_32B", "40003")),
}
clients = {}
...
def generate_new_step(problem, steps_so_far, model_size, options=None, stop_token="\n\n", temperature=0.6):
    ...
    response = client.chat.completions.create(
        ...
        messages=messages,
        temperature=temperature, top_p=0.95, # https://huggingface.co/Qwen/QwQ-32B#usage-guidelines
        max_tokens=512,
        stop=[stop_token],
...
def get_dataset(dataset_name, aime_year="2024"):
    if dataset_name == "aime":
        if aime_year == "2024":
            dataset = load_dataset("HuggingFaceH4/aime_2024")["train"]
        elif aime_year == "2025":
            from datasets import concatenate_datasets
            dataset_i = load_dataset("opencompass/AIME2025", "AIME2025-I")["test"] 
            dataset_ii = load_dataset("opencompass/AIME2025", "AIME2025-II")["test"]
            dataset = concatenate_datasets([dataset_i, dataset_ii])
        else:
            raise ValueError(f"Unsupported AIME year: {aime_year}")
    elif dataset_name == "math":
        full_dataset = load_dataset("HuggingFaceH4/MATH-500")["test"]
        random.seed(42)
        ...
        dataset = load_dataset("AI-MO/aimo-validation-amc", trust_remote_code=True)["train"]
    else:
        raise NotImplementedError
    return dataset
...
parser.add_argument("--dataset_name", type=str, choices=["aime", "math", "gpqa", "amc23"], default="gpqa",
                    help="Dataset")
parser.add_argument("--aime_year", type=str, choices=["2024", "2025"], default="2024",
                    help="AIME year (only used when dataset_name is 'aime')")
...
# problem_id: 60-89 for AIME 2024, 0-29 for AIME 2025, 0-99 for math, 0-99 for GPQA, 0-82 for AMC23
parser.add_argument("--problem_id", type=int, default=60,
                    help="Query ID (60-89 for AIME 2024, 0-29 for AIME 2025, 0-82 for AMC23)")
...
# parser.add_argument("--port_32b", type=int, default=40000,
#                     help="Port for 32B model server")
# parser.add_argument("--port_1_5b", type=int, default=40001,
#                     help="Port for 1.5B model server")
args, _ = parser.parse_known_args()

# Update ports based on command line arguments
# ports = {
#     "1.5b": args.port_1_5b,
#     "32b": args.port_32b,
# }
...
args.dataset = get_dataset(args.dataset_name, args.aime_year)

# %%
if args.dataset_name == "aime":
    # Handle different field names for different AIME years
    if args.aime_year == "2024":
        problem = args.dataset["problem"][args.problem_id - 60]
        ground_truth = args.dataset["answer"][args.problem_id - 60]
    elif args.aime_year == "2025":
        # For AIME 2025, problem_id should be 0-29 (30 problems total)
        if args.problem_id < 0 or args.problem_id >= len(args.dataset):
            raise ValueError(f"Problem ID {args.problem_id} out of range for AIME 2025 (0-{len(args.dataset)-1})")
    ...
    problem = args.dataset["Question"][args.problem_id]
    ground_truth = args.dataset["Correct Answer"][args.problem_id]
    options = {
...
try:
    while True:
        ...
        if step_id < args.first_n_steps_base_model:  # First n steps use base model
            base_model_step, finished, num_output_tokens_base = generate_new_step(problem, steps_so_far, "32b", options=options, temperature=args.temperature)
            ...
            step_str, finished, num_output_tokens = generate_new_step(problem, steps_so_far, "1.5b", options=options, temperature=args.temperature)
            ...
            if score is not None and score >= args.score_threshold:
                ...
                step_str, finished, num_output_tokens = generate_new_step(problem, steps_so_far, "32b", options=options, temperature=args.temperature)
...
# Add problem and ground truth information to metadata  
dataset_name_with_year = args.dataset_name
if args.dataset_name == "aime":
    dataset_name_with_year = f"{args.dataset_name}_{args.aime_year}"

final_metadata = {
    "problem_id": args.problem_id,
    "dataset_name": dataset_name_with_year,
    "problem_text": problem,
    "ground_truth": ground_truth,
    "options": options,
    "reasoning_steps": metadata_list,
    "total_steps": len(metadata_list),
    "total_tokens": sum([m["final_num_output_tokens"] for m in metadata_list]) if metadata_list else 0
}
...
with open(f"{output_filename}.txt", "w") as f:
    pprint.pprint(final_metadata, stream=f)
