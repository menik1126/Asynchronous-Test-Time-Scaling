...
async def compute_accuracy_from_files(output_dir, dataset_name, repeats):
    ...
    if len(generated_answers) != len(all_answers):
        print(
            f"Warning: Number of generated answers ({len(generated_answers)}) does not match number of problems in dataset ({len(all_answers)})."
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Compute accuracy from saved evaluation results."
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    asyncio.run(
        compute_accuracy_from_files(args.output_dir, args.dataset_name, args.repeats)
    )
