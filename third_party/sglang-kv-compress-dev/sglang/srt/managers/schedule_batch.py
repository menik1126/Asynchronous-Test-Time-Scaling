...
class Req:
    ...
    def __init__(
        ...
        # KV cache compression (SnapKV) state
        self.is_kv_compressed = False
        self.compressed_seq_len: Optional[int] = None
        self.original_seq_len: Optional[int] = None
        self.selected_positions: Optional[torch.Tensor] = None
        self.compress_prefix_len: Optional[int] = None
...
class ScheduleBatch:
    ...
    def mix_with_running(self, running_batch: "ScheduleBatch"):
        ...
        self.prefix_lens.extend(
            [
                (r.compressed_seq_len + len(r.output_ids) - 1 + delta
                 if getattr(r, "is_kv_compressed", False)
    ...
    def compress_kv_for_req(
        ...
        """Apply SnapKV compression to a single request's KV cache.

        Uses the last obs_window tokens (typically the most recently generated
        tokens) as the observation window to score all preceding tokens.

        If attn_weights is not provided, computes them automatically from
        the KV cache using K-as-Q approximation (no FlashAttention changes needed).

        Returns the number of freed KV slots.
        """
        ...
        selected = snapkv_select_positions(
            attn_weights, seq_len, budget, recent_window, obs_window,
        )

        return compress_request_kv(
    ...
    def copy(self):
        ...
        if hasattr(self, "snapkv_q_cache"):
            ret.snapkv_q_cache = self.snapkv_q_cache
        return ret
