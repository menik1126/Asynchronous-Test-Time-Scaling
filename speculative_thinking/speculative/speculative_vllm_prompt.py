    ...
    class ModelWorkerSingleGPU:
        ...
        def generate(self, generated_ids, sampling_params):
            outputs = self.model.generate(
                generated_ids, # prompt_token_ids=generated_ids, 
                sampling_params=sampling_params,
                use_tqdm=False
            )
            one_token = outputs[0].outputs[0].text
            one_token_id = list(outputs[0].outputs[0].token_ids)
            return one_token_id, one_token
...
def get_ray_reuslt(model, generated_ids, sampling_params):
    ...
    token_ids, one_token = ray.get(result_a_future)
    return token_ids, one_token

class spe_thinking_vllm_prompt:
            ...
            if begin or any(trigger in one_token for trigger in self.TRIGGER_TOKENS): 
                if begin:
                    ...
                    if self.help_think_word_ids is not None:
                        generated_ids_old.extend(self.help_think_word_ids)
                        generated_ids =generated_ids+self.help_think_word
                    spe_ids, spe_token = get_ray_reuslt(self.speculative_model, generated_ids, tgt_sampling_params_cache)
                    ...
                    if self.not_reasoning or spe_sent != 0:
                        ...
                        tgt_ids, tgt_token = get_ray_reuslt(self.target_model, generated_ids, tgt_sampling_params_cache)
                        ...
                        if self.choose_large or (spe_sent<0 and tgt_sent >=0) or (spe_sent>0 and tgt_sent<0):
                            ...
                            correct_tokens.append({
                                'pos': len(generated_ids_old)-prompt_len, 'token_num':self.config['max_target_tokens'],
                                'traget':tgt_token, 'speculative':spe_token})
                            # generated_ids.extend(tgt_ids)
                            final_sent=tgt_sent
                            generated_ids =generated_ids+tgt_token
                            generated_ids_old.extend(tgt_ids)
                        else:
                            # generated_ids.extend(spe_ids)
                            decode_text = spe_token
                            final_sent=spe_sent
                            generated_ids =generated_ids+spe_token
                            generated_ids_old.extend(spe_ids)
                ...
                if change_flag:
                    ...
                    tgt_ids, tgt_token = get_ray_reuslt(self.target_model, generated_ids, tgt_sampling_params_)    
                    correct_tokens.append({
                        'pos': len(generated_ids_old)-prompt_len, 'token_num':change_tokens,
                        'traget':tgt_token})
                    generated_ids =generated_ids+tgt_token
                    generated_ids_old.extend(tgt_ids)
                    change_flag = False
...
if __name__ == "__main__":
    ...
    model = spe_thinking_vllm_prompt(**config)
    messages = []
    messages.append({
        "role": "user",
