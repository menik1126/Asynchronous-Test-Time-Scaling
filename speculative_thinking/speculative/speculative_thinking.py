...
from speculative.speculative_sglang import spe_thinking_sglang
...
def load_spe_model(path):
    ...
    if config['mode'] == 'sglang':
        return spe_thinking_sglang(**config)
    if config['mode'] == 'vllm_p':
        return spe_thinking_vllm_prompt(**config)
    if config['mode'] == 'hf':
        return spe_thinking_hf(**config)
    
def process_message(message, spe_model:spe_thinking_vllm|spe_thinking_hf|spe_thinking_sglang|spe_thinking_vllm_prompt, 
...
def process_sample(idx, sample, spe_model:spe_thinking_vllm|spe_thinking_hf|spe_thinking_sglang|spe_thinking_vllm_prompt,
                    max_tokens=100, temperature=0.6, top_k=50, top_p=0.95):
