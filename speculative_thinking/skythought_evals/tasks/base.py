...
class TaskHandler(ABC):
    ...
    def _check_needs_more_samples(self, problem_data, args, temperatures):
        """
        检查问题是否需要更多采样的通用方法
        """
        if "responses" not in problem_data or args is None or temperatures is None:
            return False
            
        for temp in temperatures:
            temp_str = str(temp)
            if temp_str in problem_data["responses"]:
                # 计算已完成的采样数
                completed_samples = len([
                    r for r in problem_data["responses"][temp_str] 
                    if r and isinstance(r, dict) and r.get("content")
                ])
                
                # 如果采样数不足，需要继续处理
                if completed_samples < args.n:
                    return True
        return False
