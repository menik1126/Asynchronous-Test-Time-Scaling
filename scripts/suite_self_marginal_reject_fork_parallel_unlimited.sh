...
OUTPUT_DIR="${PROJECT_DIR}/results/self_reject_fork_unlimited_${model_short}_${DATASET_NAME}_sm${MAX_TOKENS}_t${TURNS}_temp${TEMPERATURE}_thr${PPL_THRESHOLD}_att${MAX_REJECT_ATTEMPTS}_a${ALPHA}"
mkdir -p "$OUTPUT_DIR"

echo "===================================================================================="
echo "ATTS Self-Evaluation (Rejection Sampling + Cross-Sample Forking, ALL PARALLEL, UNLIMITED)"
...
# Step 2: Compute PPL if not already available
...
no_proxy="localhost,127.0.0.1,0.0.0.0" NO_PROXY="localhost,127.0.0.1,0.0.0.0" \
    $PYTHON -m ATTS.ref_async_self_reject_fork_parallel_unlimited \
    --model_name "$MODEL" \
    --dataset_name "$DATASET_NAME" \
    --ppl_array_path "$PPL_ARRAY_PATH" \
    --model_port "$MODEL_PORT" \
...
echo ""
echo "===================================================================================="
echo "Rejection sampling + forking test completed!"
