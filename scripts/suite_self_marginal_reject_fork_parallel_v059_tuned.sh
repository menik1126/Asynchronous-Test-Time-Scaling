...
PYTHON="${PROJECT_DIR}/.sglang059/bin/python"
...
# Tuned parameters for sglang 0.5.9
PPL_THRESHOLD=0.5
...
# Reuse existing v059 PPL calibration (same model/dataset/temperature)
PPL_ARRAY_PATH="${PPL_OUTPUT_DIR}/ppls_self_prefill_${DATASET_NAME}_${model_short}_s${SAMPLE_SIZE}_t${MAX_TOKENS}_temp${TEMPERATURE}_v059.npy"
OUTPUT_DIR="${PROJECT_DIR}/results/self_reject_fork_v059_tuned_${model_short}_${DATASET_NAME}_sm${MAX_TOKENS}_t${TURNS}_temp${TEMPERATURE}_thr${PPL_THRESHOLD}_att${MAX_REJECT_ATTEMPTS}_a${ALPHA}_fg${FORK_GAP}"
mkdir -p "$OUTPUT_DIR"

echo "===================================================================================="
echo "ATTS Self-Evaluation (Rejection Sampling + Cross-Sample Forking, ALL PARALLEL)"
echo "  [sglang 0.5.9 TUNED]"
...
echo "  PPL Threshold:      $PPL_THRESHOLD  (was 0.6)"
echo "  Max Reject Attempts: $MAX_REJECT_ATTEMPTS  (was 3)"
echo "  Alpha (progress):   $ALPHA"
echo "  Fork Temperature:   $FORK_TEMPERATURE"
echo "  Fork Gap (score):   $FORK_GAP  (was 0.02)"
echo "===================================================================================="
...
echo ""
echo "[Step 3/3] Running self-async evaluation (rejection sampling + cross-sample forking)..."
HF_HUB_OFFLINE=1 \
    ...
    --fork_gap "$FORK_GAP" 2>&1 | tee "$OUTPUT_DIR/async_self_reject_fork.log"

if [ $? -ne 0 ]; then
    echo "ERROR: Rejection sampling + forking evaluation failed."
    exit 1
fi

echo ""
echo "===================================================================================="
echo "Rejection sampling + forking test completed! (sglang v0.5.9 TUNED)"
