...
OUTPUT_DIR="${PROJECT_DIR}/results/self_reject_fork_v2sys_${model_short}_${DATASET_NAME}_sm${MAX_TOKENS}_t${TURNS}_temp${TEMPERATURE}_thr${PPL_THRESHOLD}_att${MAX_REJECT_ATTEMPTS}_a${ALPHA}_b${BETA}_highppl"
mkdir -p "$OUTPUT_DIR"
...
no_proxy="localhost,127.0.0.1,0.0.0.0" NO_PROXY="localhost,127.0.0.1,0.0.0.0" \
    ...
    --fork_gap "$FORK_GAP" \
    --beta "$BETA" \
    --fork_high_ppl 2>&1 | tee "$OUTPUT_DIR/async_self_reject_fork.log"
