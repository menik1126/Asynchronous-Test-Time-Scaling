#!/bin/bash
# ATTS Self-Evaluation with SnapKV KV cache compression (K*K attention scoring)
# Uses K vectors as proxy for Q when computing importance scores.
set -eo pipefail
...
# SnapKV compression params (K*K mode: no Q caching, uses K as Q proxy)
...
PPL_ARRAY_PATH="${PPL_OUTPUT_DIR}/ppls_self_prefill_${DATASET_NAME}_${model_short}_s${SAMPLE_SIZE}_t${MAX_TOKENS}_temp${TEMPERATURE}.npy"
OUTPUT_DIR="${PROJECT_DIR}/results/self_reject_kk_compress_${model_short}_${DATASET_NAME}_sm${MAX_TOKENS}_t${TURNS}_temp${TEMPERATURE}_thr${PPL_THRESHOLD}_att${MAX_REJECT_ATTEMPTS}_b${KV_COMPRESS_BUDGET}_r${KV_COMPRESS_RATIO}"
mkdir -p "$OUTPUT_DIR"

echo "===================================================================================="
echo "ATTS Self-Evaluation (Rejection Sampling + SnapKV K*K Compression)"
...
echo "  KV Compress: budget=$KV_COMPRESS_BUDGET, ratio=$KV_COMPRESS_RATIO, mode=K*K"
echo "===================================================================================="
...
echo ""
echo "[Step 1/3] Starting model server with SnapKV K*K compression..."
env -u http_proxy -u https_proxy -u HTTP_PROXY -u HTTPS_PROXY -u no_proxy -u NO_PROXY \
    HF_HUB_OFFLINE=1 \
    SGLANG_SNAPKV_USE_CACHED_Q=0 \
...
if [ -f "$PPL_ARRAY_PATH" ]; then
    ...
    echo ""
    echo "[Step 2/3] Running self-conformal calibration..."
    env -u http_proxy -u https_proxy -u HTTP_PROXY -u HTTPS_PROXY -u no_proxy -u NO_PROXY \
...
echo ""
echo "[Step 3/3] Running self-async evaluation (rejection sampling + K*K compress)..."
no_proxy="localhost,127.0.0.1,0.0.0.0" NO_PROXY="localhost,127.0.0.1,0.0.0.0" \
    $PYTHON -m ATTS.ref_async_self_reject \
    --model_name "$MODEL" \
    --dataset_name "$DATASET_NAME" \
    --ppl_array_path "$PPL_ARRAY_PATH" \
    --model_port "$MODEL_PORT" \
    ...
    --max_reject_attempts "$MAX_REJECT_ATTEMPTS" 2>&1 | tee "$OUTPUT_DIR/async_self_reject.log"

if [ $? -ne 0 ]; then
    echo "ERROR: Rejection sampling evaluation failed."
    exit 1
fi

echo ""
echo "===================================================================================="
echo "Rejection sampling + K*K compress test completed!"
