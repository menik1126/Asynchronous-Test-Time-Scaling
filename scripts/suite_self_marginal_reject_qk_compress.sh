#!/bin/bash
# ATTS Self-Evaluation with SnapKV KV cache compression (Q*K attention scoring)
# Uses real Q vectors cached during prefill for accurate importance scores.
set -eo pipefail
...
# SnapKV compression params (Q*K mode: uses cached Q vectors from prefill)
...
OUTPUT_DIR="${PROJECT_DIR}/results/self_reject_qk_compress_${model_short}_${DATASET_NAME}_sm${MAX_TOKENS}_t${TURNS}_temp${TEMPERATURE}_thr${PPL_THRESHOLD}_att${MAX_REJECT_ATTEMPTS}_b${KV_COMPRESS_BUDGET}_r${KV_COMPRESS_RATIO}"
mkdir -p "$OUTPUT_DIR"

echo "===================================================================================="
echo "ATTS Self-Evaluation (Rejection Sampling + SnapKV Q*K Compression)"
...
echo "  KV Compress: budget=$KV_COMPRESS_BUDGET, ratio=$KV_COMPRESS_RATIO, mode=Q*K"
echo "===================================================================================="
...
echo ""
echo "[Step 1/3] Starting model server with SnapKV Q*K compression..."
...
echo ""
echo "[Step 3/3] Running self-async evaluation (rejection sampling + Q*K compress)..."
...
echo ""
echo "===================================================================================="
echo "Rejection sampling + Q*K compress test completed!"
