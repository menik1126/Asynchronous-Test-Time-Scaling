...
async def process_single_problem(
    problem, small_model_max_tokens, eval_model_port, small_model_temperature
):
    prompt = [problem, []]
    small_out = await call_small_model(
        prompt, small_model_max_tokens, small_model_temperature
    )
...
async def compute_ppl(
    ...
    for original_problem_idx, ppls in ppls_by_problem.items():
        print(
            f"problem {original_problem_idx}: {len(ppls)} samples, PPL range: [{min(ppls):.4f}, {max(ppls):.4f}]"
        )
...
async def main():
    ...
    parser.add_argument(
        "--sample_size", type=int, default=16, help="Number of samples per problem."
    )
    parser.add_argument(
        "--max_concurrent",
        type=int,
