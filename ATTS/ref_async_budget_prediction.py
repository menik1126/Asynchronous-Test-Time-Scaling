        ...
        try:
            async with semaphore:
                resp = await client_eval.post(
                    f"http://127.0.0.1:{port}/generate", json=payload, timeout=60.0
                )
                resp.raise_for_status()
                data = resp.json()
                input_token_logprobs = data["meta_info"]["input_token_logprobs"][1:]
                logprobs = [
                    entry[0] for entry in input_token_logprobs if entry[0] is not None
                ]
                if not logprobs:
                    print(
                        f"No log probabilities returned for problem: {prompt[0]}",
                        flush=True,
                    )
                    return 0
...
def should_takeover_based_on_percentile(ppl_value):
    ...
    if ppl_array is not None:
        ...
        print(
            f"compare ppl: current={ppl_value:.4f}, range=[{min_ppl:.4f}, {max_ppl:.4f}], rank={rank}/{len(ppl_array)}"
        )
    ...
    if should_takeover:
        print(
            f"take over PPL={ppl_value:.4f}, percentage={percentile:.3f} >= {percentile_threshold}"
        )
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                wait_time = (attempt + 1) * 2
                print(
                    f"small model error (try {attempt + 1}/{max_retries}): {e}. wait {wait_time}s ...",
                    flush=True,
                )
                await asyncio.sleep(wait_time)
            else:
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                wait_time = (attempt + 1) * 2
                print(
                    f"eval model error (try {attempt + 1}/{max_retries}): {e}. wait {wait_time}s ...",
                    flush=True,
                )
                await asyncio.sleep(wait_time)
            else:
                print(f"eval model (sample {idx}): {e}", flush=True)
                return ""
        except Exception as e:
...
async def process_single_problem(
    ...
    for turn in range(turns):
        ...
        small_out = await call_small_model(
            prompt, turn, small_model_max_tokens, idx, small_model_port
        )
        ...
        history_log.append(
            {
                ...
                "model": "eval_ppl",
                "ppl": ppl,
                "should_takeover": int(should_takeover),
            }
        )
        ...
        if should_takeover:
            ...
            print(
                f"Turn {turn+1}: doesn't take over (Sample {idx}, PPL={ppl:.4f})",
                flush=True,
            )
            ...
            if is_last_sample_in_group and (is_last_turn or is_early_stop):
                ...
                for sample_idx in range(start_sample_idx, end_sample_idx):
                    if sample_idx in takeover_stats.get("sample_takeovers", {}):
                        current_group_takeovers += takeover_stats["sample_takeovers"][
                            sample_idx
                        ]

                accuracy = (
                    1
                    - abs(current_group_takeovers - theoretical_takeovers)
                    / theoretical_takeovers
                )
                print(
                    f"Group {problem_group_idx} finish - theory={theoretical_takeovers}, practical={current_group_takeovers}, accuracy={accuracy:.1%}",
                    flush=True,
                )
...
async def compute_score(
    results, answers, repeats, takeover_stats=None, takeover_budget=None
):
    ...
    for i in range(group):
        ...
        print(f"group {i}: correct={correct_answer}, output={outputs}, match={matched}")
...
async def main():
    ...
    if not unique_problems_to_process:
        ...
        await compute_score(
            all_results, answer, args.repeats, takeover_stats, args.takeover_budget
        )
        return
    ...
    total_problems_processed = len(takeover_stats["problems_with_takeover"]) + len(
        takeover_stats["problems_without_takeover"]
    )
    print(f"Total problem groups processed: {total_problems_processed}")
    print(
        f"Problem groups with takeover: {len(takeover_stats['problems_with_takeover'])}"
    )
    print(
        f"Problem groups without takeover: {len(takeover_stats['problems_without_takeover'])}"
    )
    print(f"Total takeover count: {takeover_stats['total_takeovers']}")

    if takeover_stats["problems_without_takeover"]:
        print(
            f"Problem groups without takeover: {sorted(takeover_stats['problems_without_takeover'])}"
        )

        print("\n" + "=" * 60)
        print("Details of problem groups without takeover")
        print("=" * 60)

        for detail in takeover_stats["no_takeover_details"]:
            print(
                f"Problem group {detail['problem_group_idx']} (sample {detail['sample_idx']}):"
            )
            print(f"Question: {detail['question']}")
            print(f"PPL values: {[f'{p:.4f}' for p in detail['ppl_values']]}")
            print(
                f"Average PPL: {sum(detail['ppl_values'])/len(detail['ppl_values']):.4f}"
            )
            print("-" * 40)

    if total_problems_processed > 0:
        takeover_rate = (
            len(takeover_stats["problems_with_takeover"])
            / total_problems_processed
            * 100
        )
