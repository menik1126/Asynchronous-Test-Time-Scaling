...
# PPL数组数据（用于百分位数计算）
ppl_array = None
percentile_threshold = None  # 百分位数阈值，默认50%分位数
...
def load_ppl_array(ppl_array_path):
    ...
    if ppl_array_path and os.path.exists(ppl_array_path):
        ppl_array = np.load(ppl_array_path, allow_pickle=True).item()  # 加载字典格式
        print(f"✅ 成功加载PPL数组数据: 字典格式，包含 {len(ppl_array)} 个问题")
        # 打印每个问题的PPL数量
        for problem_idx, ppls in ppl_array.items():
            print(f"  问题 {problem_idx}: {len(ppls)} 个PPL值")
        return True
    else:
...
def should_takeover_based_on_percentile(ppl_value, problem_group_idx):
    ...
    if problem_group_idx not in ppl_array:
        print(f"⚠️  问题组 {problem_group_idx} 在PPL数据中不存在")
        return False
    
    current_problem_ppls = ppl_array[problem_group_idx]
    print(f"🔍 当前问题组PPL数据: {current_problem_ppls}")
    print(f"🔍 当前PPL值: {ppl_value}")
    
    # 将列表转换为numpy数组以支持向量化操作
    current_problem_ppls_array = np.array(current_problem_ppls)
    
    # 计算当前PPL值在对应问题组PPL分布中的排名
    ...
    print(f"🔍 PPL对比 (问题组 {problem_group_idx}): 当前={ppl_value:.4f}, 历史范围=[{min_ppl:.4f}, {max_ppl:.4f}], 排名={rank}/{len(current_problem_ppls_array)}")
...
async def call_eval_model(prompt, max_tokens, idx, port):
    ...
    try:
        ...
        print(f"❌ 评估模型调用失败 (样本 {idx}): {e}", flush=True)
        return ""
...
async def main():
    ...
    if ppl_array is not None:
        print(f"📊 PPL数组信息: 字典格式，包含 {len(ppl_array)} 个问题")
        # 统计每个问题的PPL数量
        total_ppl_samples = sum(len(ppls) for ppls in ppl_array.values())
        print(f"📊 总PPL样本数: {total_ppl_samples}")
        # 统计百分位数分布
        expected_takeover_count = int(total_ppl_samples * percentile_threshold)
        print(f"📊 预期接管样本数: {expected_takeover_count}/{total_ppl_samples} ({percentile_threshold*100:.1f}%分位数)")
