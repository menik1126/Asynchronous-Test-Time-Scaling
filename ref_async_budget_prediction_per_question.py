...
def should_takeover_based_on_percentile(ppl_value, problem_group_idx):
    ...
    print(
        f"compare ppl (group {problem_group_idx}): current={ppl_value:.4f}, range=[{min_ppl:.4f}, {max_ppl:.4f}], rank={rank}/{len(current_problem_ppls_array)}"
    )
...
async def process_single_problem(
    ...
    for turn in range(turns):
        ...
        if should_takeover:
            print(
                f"Turn {turn+1}: take over! (Sample {idx}, PPL={ppl:.4f})", flush=True
            )
            ...
            print(
                f"Turn {turn+1}: not take over (Sample {idx}, PPL={ppl:.4f})",
                flush=True,
            )
        ...
        if takeover_stats and takeover_budget is not None:
            ...
            if is_last_sample_in_group and (is_last_turn or is_early_stop):
                ...
                print(
                    f"Group {problem_group_idx} finish - theory={theoretical_takeovers}, practical={current_group_takeovers}次, accuracy={accuracy:.1%}",
                    flush=True,
                )
